...
volatile int adc_anterior = 0;

static void callback_data(void){
	// Coleta Dados e passa eles para fila Bruto
	adcData adc;
	adc.value = afec_channel_get_value(AFEC_POT, AFEC_POT_CHANNEL);
	BaseType_t xHigherPriorityTaskWoken = pdTRUE;
	printf("Adiciondo a fila\n");
	xQueueSendFromISR(xQueueBruto, &adc, &xHigherPriorityTaskWoken);
}

static void task_proc(void *pvParameters) {
	BaseType_t xHigherPriorityTaskWoken = pdTRUE;
	// configura ADC e TC para controlar a leitura
	config_AFEC_pot(AFEC_POT, AFEC_POT_ID, AFEC_POT_CHANNEL, callback_data);
	...
	while (1) {
		...
		if (xQueueReceive(xQueueBruto, &adc, 1000)) {
			printf("ADC: %d \n", adc.value);
			soma_movel += adc.value;
			} else {
			printf("Nao chegou ADC");
			soma_movel += 0;
		}
		
		contador_media += 1;
		if (contador_media >= 10){
			media_movel = soma_movel / 10;
			xQueueSend(xQueueADC, &media_movel, &xHigherPriorityTaskWoken);
			contador_media = 0;
			soma_movel = 0;
		}
	}
...
static void task_adc(void *pvParameters) {
	...
	int movel = 0;
	while (1) {
		if (xQueueReceive(xQueueADC, &movel, 1000)) {
...
int main(void) {
	...
	xQueueBruto = xQueueCreate(100, sizeof(adcData));
	if (xQueueADC == NULL)
