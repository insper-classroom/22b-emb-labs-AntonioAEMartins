...
void but_callback(void) {
	...
	int var = 0;
	xQueueSendFromISR(xQueueSemaforo, (void *)&var, 10);
}
...
static void task_but(void *pvParameters) {
	...
	for (;;) {
		if (xQueueReceive(xQueueSemaforo, &msg, (TickType_t) 0)) {
			if (msg == 1){
				printf("Aumentou Frequencia\n");
				delayTicks += 100;
				delayTicks = delayTicks / portTICK_PERIOD_MS;
				xQueueSend(xQueueLedFreq, (void *)&delayTicks, 10);
			} if (msg == 0){
				printf("Diminuiu Frequencia\n");
				delayTicks -= 100;
				delayTicks = delayTicks / portTICK_PERIOD_MS;
				xQueueSend(xQueueLedFreq, (void *)&delayTicks, 10);
			}
...
void BUT_init(void) {
	...
	pio_handler_set(BUT1_PIO, BUT1_PIO_ID, BUT1_IDX_MASK, PIO_IT_FALL_EDGE, &but2_callback);
...
int main(void) {
	...
	xQueueSemaforo = xQueueCreate(32, sizeof(uint32_t));
	if (xQueueSemaforo == NULL){
		printf("falha em criar a queue \n");
	}
